...
const one = (arr, base) => {
    ...
    while (!foundedNumber && arr.length) {
        currentNumber = arr.shift()
        foundedNumber = arr.find(number => base + number + currentNumber === 2020)
    }

    return currentNumber * foundedNumber * (base !== 0 ? base : 1)
}
...
const two = arr => {
    ...
    while (!foundedNumber && arr.length > 2) {
        currentNumber = arr.shift()
        foundedNumber = one([...arr], currentNumber)
    }
